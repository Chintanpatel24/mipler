...
export const useWorkspaceStore = create<WorkspaceStore>((set, get) => ({
  ...
  removeInvestigation: (id) => {
    ...
    const newActive = id === s.activeInvestigationId ? remaining[0] : s.investigations.find((i) => i.id === s.activeInvestigationId) || remaining[0];
    set({
  ...
  combineInvestigations: () => {
    ...
    for (const inv of s.investigations) {
      for (const node of inv.nodes) {
        allNodes.push({ ...node, position: { x: node.position.x + offsetX, y: node.position.y } });
      }
      for (const edge of inv.edges) {
        allEdges.push({ ...edge });
      }
      const maxX = inv.nodes.reduce((max, n) => Math.max(max, n.position.x + (n.data.width || 300)), 0);
      offsetX += maxX + 200;
    }
